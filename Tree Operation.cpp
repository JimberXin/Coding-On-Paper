...
//Modify: 2014/11/26: fix the format in VS
...
vector<int> postOrder(TreeNode* root){
      ...
      while(p !=NULL && !nodeStack.empty()){
	    ...
	    p = nodeStack.top();
   //if the current node 'p' has no right child, or has visited, visit p
...
vector<int> DFS(TreeNode* root){
	...
	while(!nodeStack.empty() ){
	      ...
	      result.push_back(p -> val);
	      nodeStack.pop();
	      if(p->right != NULL) nodeStack.push(p -> right);
	      if(p ->left  != NULL) nodeStack.push(p -> left);
	}
	return result;
}
...
//****find the given key in the tree*****Iterative & Non-Iteration********
...
TreeNode *buildTreeFromInandPost(vector<int>& inOrder, vector<int>&postOrder){
   ...
   return buildHelper(inOrder.begin(), inOrder.end(),
...
TreeNode* buildTreeFromInandPre(vector<int>& inOrder, vector<int>& preOrder){
  if(inOrder.size() == 0) return NULL;
  return buildInandPreHelper(inOrder.begin(),  inOrder.end(),
